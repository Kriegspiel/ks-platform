...
- `.site-refresh`: detached worktrees used by the hourly static-site refresh job
- `_tmp`: local holding area for non-active repos/worktrees
- `node_modules`: top-level Codex CLI dependency install
- `localsetup`: local machine notes such as Cloudflare tunnel setup
...
That rewrites [`documentation/module-index.md`](./documentation/module-index.md) from the checked-out active repos in the local workspace.
